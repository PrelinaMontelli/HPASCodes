...
/**
  * @brief  OLED显示浮点数（十进制，带小数点）
  * @param  Line 起始行位置，范围：1~4
  * @param  Column 起始列位置，范围：1~16
  * @param  Number 要显示的数字
  * @param  DecimalLength 小数位数，范围：1~6
  * @retval 无
  */
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t DecimalLength)
{
	char buf[24];
	uint32_t pow10 = OLED_Pow(10, DecimalLength);
	int32_t scaled = (int32_t)(Number * pow10);
	int32_t int_part = scaled / (int32_t)pow10;
	uint32_t frac_part = (uint32_t)(scaled < 0 ? -scaled : scaled) % pow10;

	OLED_ShowString(Line, Column, "                ");
	snprintf(buf, sizeof(buf), "%ld.%0*lu", (long)int_part, DecimalLength, (unsigned long)frac_part);
